...
def template(prompts, systems=None, mode='llama3'):
    if mode == 'llama3':
        results = []
        systems = systems or [''] * len(prompts)  # Default to empty strings if systems is None or shorter than prompts
...
def inferSystemModelVllm(model, prompts, system_prompts="Act as a Mario in Super Mario Brothers", mode='llama3', temperature=0.8, top_p=0.9, max_tokens=2000):
    if not isinstance(system_prompts, list):
        system_prompts = [system_prompts] * len(prompts)

    if len(system_prompts) != len(prompts):
        raise ValueError("The length of system_prompts must match the length of prompts.")

    sampling_params = SamplingParams(temperature=temperature, top_p=top_p, max_tokens=max_tokens)
    
    llm = LLM(model=f"../text-generation-webui/models/{model}", dtype='half', max_model_len=8192)
...
def inferModelVllm(model, prompts, temperature=0.8, top_p=0.9, max_tokens=2000):
    sampling_params = SamplingParams(temperature=temperature, top_p=top_p, max_tokens=max_tokens)
