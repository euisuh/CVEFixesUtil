...
for i in tqdm(range(df.shape[0])):
    res_before = inferModel(promptR1(df.code_before.iloc[i], cwe_map.get(df['exp_cwe_id'].iloc[i][4:])['name']))
    res_after = inferModel(promptR1(df.code_after.iloc[i], cwe_map.get(df['exp_cwe_id'].iloc[i][4:])['name']))
    
    df.at[i, 'res_before'] = res_before
    df.at[i, 'res_after'] = res_after
    
df.to_csv('kat_res/res_R1_llama3.csv', index=False)  